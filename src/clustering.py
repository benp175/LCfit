...
def clustering(sampler, state, names, likelihood, obsdf, ndim, nwalkers, const = 50, lag = 10, max_prune_frac = 0.9, reburnin = 100):
	...
	llhoodparam = pd.DataFrame(columns = ['llhood'] + names)
...
#	if verbose:
	...
	if freject < max_prune_frac:
		...
		sampler = emcee.EnsembleSampler(nwalkers, ndim, likelihood, args = [obsdf], live_dangerously = True)
